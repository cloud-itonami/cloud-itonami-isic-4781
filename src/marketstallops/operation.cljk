(ns marketstallops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (MarketStallRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the MarketStallRetailGovernor
  ...
  human market-stall operations coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [marketstallops.advisor :as advisor]
            [marketstallops.governor :as governor]
            [marketstallops.phase :as phase]
...
(defn build
  ...
  `marketstallops.store/Store`).
  opts:
    :advisor      -- a `marketstallops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MarketStallRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; MarketStallRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
