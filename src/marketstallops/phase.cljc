(ns marketstallops.phase
  "Phase 0->3 staged rollout for the ISIC-4781 stall/market food, beverage
  and tobacco retail operations-coordination actor.

    Phase 0  read-only            -- no writes, still governor-gated.
    Phase 1  assisted-logging     -- sales-record logging allowed, every
                                     write needs human approval.
    Phase 2  assisted-coordination-- adds stall-operation scheduling
  ...
  suspected food-safety-clearance failure or age-verification failure)
  always needs a human to actually look at it -- this op never itself
  finalizes a food-safety clearance or an age-verification override.
  `marketstallops.governor`'s own `always-escalate-ops` enforces the same
  invariant independently -- two layers, not one, agree on this."
  (:require [marketstallops.governor :as governor]))
...
(defn verdict->disposition
  "Map a MarketStallRetailGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
