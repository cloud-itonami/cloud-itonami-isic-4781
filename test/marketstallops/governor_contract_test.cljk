(ns marketstallops.governor-contract-test
  ...
  (testing "clean sales-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-sales-record :stall-id "stall-1" :patch {:units-sold 24 :item "grilled skewers"}}
                               ctx)]
      (is (some? result))
  ...
  (testing ":flag-compliance-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-compliance-concern :stall-id "stall-1"
                                :patch {:concern "suspected underage tobacco purchase attempt" :confidence 0.99}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :stall-id "stall-1"
                                :patch {:item "bulk tobacco product restock" :quantity 1 :estimated-cost 2400.0}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into food-safety-clearance/age-verification-override scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8a"
                     {:op :log-sales-record :stall-id "stall-1" :patch {:units-sold 24}}
                     ctx)
      (exec-request actor "t8b"
                     {:op :log-sales-record :stall-id "unknown" :patch {:units-sold 24}}
                     ctx)
